...
요청 제한은 주로 `limit_req_zone`과 `limit_req` 두 가지 지시어를 사용하여 설정됩니다. 아래는 그 예시입니다:
...
`limit_req_zone` 지시어는 요청 제한을 위한 매개변수를 정의하며, `limit_req`는 해당 지시어가 위치한 컨텍스트(예시에서는 /login/로의 모든 요청) 내에서 요청 제한을 활성화합니다.
...
## Zone
각 IP 주소의 상태와 요청 제한 URL에 얼마나 자주 액세스 되었는지를 저장하는 공유 메모리 영역을 정의합니다. 공유 메모리에서 정보를 유지하면 NGINX 작업 프로세스 간에 정보를 공유할 수 있습니다. 정의는 두 부분으로 이루어집니다: zone= 키워드로 식별되는 영역 이름과 콜론 뒤의 크기. 대략 16,000개의 IP 주소에 대한 상태 정보는 1메가바이트를 차지하므로, 우리의 영역은 대략 160,000개의 주소를 저장할 수 있습니다.

저장 공간이 부족할 때 NGINX는 새로운 항목을 추가해야 할 때 가장 오래된 항목을 제거합니다. 여전히 새 레코드를 수용하기에 충분한 공간이 확보되지 않으면, NGINX는 상태 코드 503(서비스 일시적으로 사용 불가)을 반환합니다. 또한, 메모리가 고갈되는 것을 방지하기 위해 NGINX가 새 항목을 생성할 때마다 이전 60초 동안 사용되지 않은 최대 두 개의 항목을 제거합니다.

## Rate 

최대 요청 속도를 설정합니다. 예시에서는 초당 10회의 요청을 초과할 수 없습니다. 실제로 NGINX는 밀리초 단위로 요청을 추적하므로, 이 제한은 100 밀리초마다 1회의 요청에 해당합니다. 우리가 급증(bursts)을 허용하지 않기 때문에, 이는 이전 허용된 요청 후 100ms 이내에 요청이 도착하면 요청이 거부된다는 것을 의미합니다.

---

`limit_req_zone` 지시어는 요청 제한 및 공유 메모리 영역의 매개변수를 설정하지만 실제로 요청 속도를 제한하지는 않습니다. 이를 위해서는 특정 위치나 서버 블록에 limit_req 지시어를 포함하여 제한을 적용해야 합니다. 예시에서는 `/login/`로의 요청을 제한하고 있습니다.
...
이 효과는 초당 10회의 요청 제한과 동일합니다. nodelay 옵션은 요청 간 허용된 간격에 제약을 받지 않고 Rate Limit을 적용하려는 경우 유용합니다.
